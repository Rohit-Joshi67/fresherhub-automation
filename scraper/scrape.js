...
async function run() {
  ...
  try {
    // Poster images are generated by the assistant pipeline (Drive delivery),
    // not here — keep the text queue (captions/headlines) which feeds it.
    const { generatePhotos } = await import('./generate-photos.js');
    const n = await generatePhotos({ jobs: finalJobFeed, repoRoot, dataDir, maxPerRun: 0 });
